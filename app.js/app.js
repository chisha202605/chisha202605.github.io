...
function renderDish(){
  ...
  dishes.filter(d=d.name.includes(keyword)).forEach(d={
    list.innerHTML += `
      div class=dish-item
        divb${d.name}b ${d.diff} · ${d.time}div
        div分类：${getCateName(d.cateId)}div
        button onclick=deleteDish('${d.id}')删除button
      div`
  })
}
...
function saveDish(){
  ...
  dishes.push({
    idDate.now().toString(),name,cateId,diff,time,people,ing,step,video,
    isDeletefalse,deleteTimenull
  })
...
function deleteDish(id){
 ...
 随机配餐 4菜1汤 必含荤菜 3天不重复
function randomMeal(){
  const dishes = JSON.parse(localStorage.getItem('dishes')).filter(d=!d.isDelete)
  const usedIds = JSON.parse(localStorage.getItem('usedDishIds'))
  const threeDay = Date.now()-32460601000
  const validUsed = usedIds.filter(x=x.timethreeDay).map(x=x.id)
  ...
  if(meat.length1soup.length1other.length3)return alert('菜品不足或3天内重复限制')
  ...
  const meal = [m,...o,s]
   记录已用
  usedIds.push(...meal.map(x=({idx.id,timeDate.now()})))
...
function renderMeal(meal){
  let html = ''
  meal.forEach(d={
    html += `div class=dish-itemb${d.name}bdiv`
    const ings = d.ing.split('n').filter(i=i.trim())
...
function showRecycle(){
  ...
  let html = ''
  dishes.forEach(d={
    const day = (Date.now()-d.deleteTime)1000606024
    html += `div class=dish-item${d.name} ${day7'已过期''可恢复'}
      button onclick=restoreDish('${d.id}') ${day7'disabled'''}恢复button
    div`
  })
...
function exportData(){
  const data = JSON.stringify({
    categoriesJSON.parse(localStorage.getItem('categories')),
    dishesJSON.parse(localStorage.getItem('dishes')),
    mealHistoryJSON.parse(localStorage.getItem('mealHistory'))
  })
